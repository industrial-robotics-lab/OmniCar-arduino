...
inline float rotate(float angle, float desiredAngle) {
...
inline bool is_near_zero(float s) {
    return abs(s) < 1e-6;
}

inline float norm(Matrix<3> v) {
    return sqrt(v(0)*v(0) + v(1)*v(1) + v(2)*v(2));
}

inline Matrix<3> normalize(Matrix<3> v) {
    float nor = norm(v);
    if (nor == 0) return v;
    return v / nor;
}

inline Matrix<3,3> skew(Matrix<3> v) {
    Matrix<3,3> skew = {0, -v(2), v(1), v(2), 0, -v(0), -v(1), v(0), 0};
    return skew;
}

inline Matrix<4,4> vec6_to_se3(Matrix<6> v) {
...
inline Matrix<3> so3_to_vec(Matrix<3,3> so3) {
    Matrix<3> vec = {so3(2,1), so3(0,2), so3(1,0)};
    return vec;
}

inline Matrix<3,3> so3_to_SO3(Matrix<3,3> so3) {
...
inline Matrix<4,4> se3_to_SE3(Matrix<4,4> se3) {
